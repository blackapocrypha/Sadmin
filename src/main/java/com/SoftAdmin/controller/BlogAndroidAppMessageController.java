...
@Controller
@RequestMapping("/BAndroidAppMessage")
public class BlogAndroidAppMessageController {
	...
	@ResponseBody
	public List<BlogAapptalk> getAllToolMessage(Integer number){
		number=page;
		Integer beforelimit = (number-1)*5;
		Integer afterlimit = 5;
		List<BlogAapptalk> btalk = blogAndroidAppMessageService.getAllToolMessage(beforelimit,afterlimit);
		return btalk;
	}
	...
	public Integer howmanybooks() {
		Integer nu = (blogAndroidAppMessageService.howmanymessages()+4)/5;
		if(nu>5) {nu=5;}
		return nu;
	}
	...
	public String changePage(Integer num) {
		page=num;
		return "redirect:/Blog_androidappMessage.jsp";
	}
	...
	public BlogAapptalk memberedit(Integer nid) {
		BlogAapptalk btalk = blogAndroidAppMessageService.memberedit(nid);
		return btalk;
	}
	...
	//跟据id更新
		...
		@ResponseBody
		public List<BlogAapptalk> substringBy(String begintime,String endtime) {
			...
			//获取所有日期
			List<BlogAapptalk> datalist = blogAndroidAppMessageService.selectAllIdAndTime();
			//声明保存id数组
			...
			if(idlist.isEmpty()) {
				...
				//查询指定id的留言
				List<BlogAapptalk> selectmessage =  blogAndroidAppMessageService.selectAllChosenByid(idlist);
