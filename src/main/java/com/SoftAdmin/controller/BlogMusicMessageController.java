...
@Controller
@RequestMapping("/BmusicM")
public class BlogMusicMessageController {
	...
	//获取所有留言
		...
		@ResponseBody
		public List<BlogMusictalk> getAllToolMessage(Integer number){
			number=page;
			Integer beforelimit = (number-1)*5;
			Integer afterlimit = 5;
			List<BlogMusictalk> btalk = blogMusicMessageService.getAllToolMessage(beforelimit,afterlimit);
			return btalk;
		}
		...
		public Integer howmanybooks() {
			Integer nu = (blogMusicMessageService.howmanymessages()+4)/5;
			if(nu>5) {nu=5;}
			return nu;
		}
		...
		public String changePage(Integer num) {
			page=num;
			return "redirect:/Blog_musicMessage.jsp";
		}
		...
		public BlogMusictalk memberedit(Integer nid) {
			BlogMusictalk btalk = blogMusicMessageService.memberedit(nid);
			return btalk;
		}
		...
		@ResponseBody
		public String updatemessageByid(BlogMusictalk bt) {
			Boolean flag = blogMusicMessageService.updatemessageByid(bt);
			if(flag){
				return "1";
			}else{
				return "0";
		...
		@ResponseBody
		public List<BlogMusictalk> substringBy(String begintime,String endtime) {
			...
			//获取所有日期
			List<BlogMusictalk> datalist = blogMusicMessageService.selectAllIdAndTime();
			//声明保存id数组
			...
			if(idlist.isEmpty()) {
				...
				//查询指定id的留言
				List<BlogMusictalk> selectmessage =  blogMusicMessageService.selectAllChosenByid(idlist);
		...
		public String deletemessage(Integer nid) {
			Boolean flag = blogMusicMessageService.deletemessage(nid);
			if(flag) {
				return "1";
			}else {
				return "0";
