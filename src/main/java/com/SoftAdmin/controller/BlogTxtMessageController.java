...
@Controller
@RequestMapping("/Btxtmessage")
public class BlogTxtMessageController {
	...
	//获取所有留言
			...
			@ResponseBody
			public List<BlogTxttalk> getAllToolMessage(Integer number){
				number=page;
				Integer beforelimit = (number-1)*5;
				Integer afterlimit = 5;
				List<BlogTxttalk> btalk = blogTxtMessageService.getAllToolMessage(beforelimit,afterlimit);
				return btalk;
			}
			...
			public Integer howmanybooks() {
				Integer nu = (blogTxtMessageService.howmanymessages()+4)/5;
				if(nu>5) {nu=5;}
				return nu;
			}
			...
			public String changePage(Integer num) {
				page=num;
				return "redirect:/Blog_txtMessage.jsp";
			}
			...
			public BlogTxttalk memberedit(Integer nid) {
				BlogTxttalk btalk = blogTxtMessageService.memberedit(nid);
				return btalk;
			}
			...
			//跟据id更新
				...
				@ResponseBody
				public List<BlogTxttalk> substringBy(String begintime,String endtime) {
					...
					//获取所有日期
					List<BlogTxttalk> datalist = blogTxtMessageService.selectAllIdAndTime();
					//声明保存id数组
					...
					if(idlist.isEmpty()) {
						...
						//查询指定id的留言
						List<BlogTxttalk> selectmessage =  blogTxtMessageService.selectAllChosenByid(idlist);
