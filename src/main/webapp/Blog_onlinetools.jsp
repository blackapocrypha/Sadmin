...
<html class="x-admin-sm">
   ...
   function searchmessage(fid){//searchmessage start
   	...
   	$.ajax({//ajax begin
		type:'GET',
     	url:'${pageContext.request.contextPath}/Blog/getAllBlogOnlineTools?number='+fid,
     	dataType:'json',
     	async: false,
     	success:function(rs){ 		
     		$.each(rs,function(index,item){
               	...
               	$('#tbmessage').append("<tr>"
                     ...
                     +"<td>"+item.nid+"</td>"
                     +"<td>"+item.webname+"</td>"
                     +"<td><a target='_blank' href='"+item.weburl+"'>"+item.weburl+"</a></td>"
                     +"<td>"+item.webdis+"</td>"
                     +"<td class='td-manage'>"
                     
                     +"<a title='编辑'  onclick=\"xadmin.open('编辑','Blog_onlinetools-edit.jsp?nid="+item.nid+"',600,400)\" href='javascript:;'>"
   ...
   function getcount(){
	   ...
	   $.ajax({
			type:'GET',
	     	url:'${pageContext.request.contextPath}/Blog/howmanywebs',
	     	dataType:'json',
	     	async: false,
	     	success:function(rs){ 
	     		usersnumber = rs;
   ...
   $.ajax({
	   	type:"GET",
	   	url:"${pageContext.request.contextPath}/Blog/returnPage",
	   	datatype:"json",
	   	async: false,
	   	success:function(rs){
	   		if(rs==1){
	   			...
	   			if(parseInt(usersnumber)<2){
	   				...
	   				$("#countpage").html("<a class='prev'  id='prev' onclick='goprev()'  href='javascript:;'>&lt;&lt;</a>"
	   						+"<a class='num'  href='Blog/changePage?num=1'>首页</a>"        
	   						+"<span class='current' id='currentthis'>"+rs+"</span>"
	                        +"<a class='num' onclick='chosenthis("+(parseInt(rs)+1)+")' href='Blog/changePage?num="+(parseInt(rs)+1)+"'>"+(parseInt(rs)+1)+"</a>"
	                        +"<a class='num' onclick='chosenthis("+(parseInt(rs)+2)+")'  href='Blog/changePage?num="+(parseInt(rs)+2)+"'>"+(parseInt(rs)+2)+"</a>"
	                        +"<a class='num'  href='Blog/changePage?num="+usersnumber+"'>尾页</a>" 
	   			...
	   			$("#countpage").html("<a class='prev' id='prev' onclick='goprev()' href=''>&lt;&lt;</a>"
	   				    ...
	   				    +"<a class='num' onclick='chosenthis("+(parseInt(rs)-1)+")' href='Blog/changePage?num="+(parseInt(rs)-1)+"'>"+(parseInt(rs)-1)+"</a>"
                        +"<span class='current' id='currentthis'>"+rs+"</span>"
                        +"<a class='num'  href='Blog/changePage?num="+(parseInt(rs)+1)+"'>"+(parseInt(rs)+1)+"</a>" 
                        +"<a class='num'  href='Blog/changePage?num="+usersnumber+"'>尾页</a>" 
                    ...
                    <div class="layui-card">
                        <div class="layui-card-body ">
                            <form class="layui-form layui-col-space5">
                                ...
                                <div class="layui-inline layui-show-xs-block">
                                    <input type="text"  id="mohusearch" placeholder="请输入网站名"  class="layui-input">
                                </div>
                                <div class="layui-inline layui-show-xs-block">
                        ...
                        <div class="layui-card-header">
                            ...
                            <button class="layui-btn" onclick="xadmin.open('添加用户','./Blog_onlinetools-add.jsp',600,400)"><i class="layui-icon"></i>添加</button>
                        </div>
 ...
 function gonext(){
	...
	$("#next").attr("href","Blog/changePage?num="+currentthis);
	}	
 }
 ...
 //回退翻页
  function goprev(){
	...
	if(currentthis<=1){
	$("#prev").attr("href","Blog/changePage?num="+currentthis);
 ...
 $("#searchbutton").click(function(){ 
	 ...
	 $.ajax({		 
		 type:"POST",
		 url:"${pageContext.request.contextPath}/Blog/selectWebByName",
		 data:{"webname":mohusearch},
		 async:false,
		 dataType:"json",
		 success:function(rs){
			 $('#tbmessage').html("");
			 ...
			 $('#tbmessage').append("<tr>"
                     ...
                     +"<td>"+rs.nid+"</td>"
                     +"<td>"+rs.webname+"</td>"
                     +"<td><a target='_blank' href='"+rs.weburl+"'>"+rs.weburl+"</a></td>"
                     +"<td>"+rs.webdis+"</td>"
                     +"<td class='td-manage'>"    
                     +"<a title='编辑'  onclick=\"xadmin.open('编辑','Blog_onlinetools-edit.jsp?nid="+rs.nid+"',600,400)\" href='javascript:;'>"
                     +" <i class='layui-icon'>&#xe642;</i> </a>"
