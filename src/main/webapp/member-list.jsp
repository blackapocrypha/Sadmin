...
<html class="x-admin-sm">
   ...
   function searchmessage(fid){//searchmessage start
   	...
   	$.ajax({//ajax begin
		type:'GET',
     	url:'${pageContext.request.contextPath}/users/getAllUsersList?number='+fid,
     	dataType:'json',
     	async: false,
     	success:function(rs){ 		
     		$.each(rs.users,function(index,item){
     		   $.each(rs.udetils,function(index,it){	
                	if(it.uphone==item.username){
                		sex = it.usex;
                		uage = it.uage;
                		umail = it.umail;
                		ufname = it.ufname;
               	...
               	$('#tbmessage').append("<tr>"
                     ...
                     +"<a title='编辑'  onclick=\"xadmin.open('编辑','member-edit.jsp?username="+item.username+"',600,400)\" href='javascript:;'>"
                     +" <i class='layui-icon'>&#xe642;</i> </a>"
                     +"<a onclick=\"xadmin.open('修改密码','member-password.jsp?username="+item.username+"',600,400)\" title='修改密码' href='javascript:;'>"
                     +" <i class='layui-icon'>&#xe631;</i> </a>"
   ...
   $.ajax({
	   	type:"GET",
	   	url:"${pageContext.request.contextPath}/users/returnPage",
	   	datatype:"json",
	   	async: false,
	   	success:function(rs){
	   		if(rs==1){
	   			...
	   			if(parseInt(usersnumber)<2){
	   				...
	   				$("#countpage").html("<a class='prev'  id='prev' onclick='goprev()'  href='javascript:;'>&lt;&lt;</a>"
	                        +"<span class='current' id='currentthis'>"+rs+"</span>"
	                        +"<a class='num' onclick='chosenthis("+(parseInt(rs)+1)+")' href='users/changePage?num="+(parseInt(rs)+1)+"'>"+(parseInt(rs)+1)+"</a>"
	                        +"<a class='num' onclick='chosenthis("+(parseInt(rs)+2)+")'  href='users/changePage?num="+(parseInt(rs)+2)+"'>"+(parseInt(rs)+2)+"</a>"                          
 ...
 function gonext(){
	...
	$("#next").attr("href","users/changePage?num="+currentthis);
	}	
 }
 ...
 //回退翻页
  function goprev(){
	...
	if(currentthis<=1){
	$("#prev").attr("href","users/changePage?num="+currentthis);
...
for(var i=1;i<=4;i++){
	if(obj==i){
		...
		$("#countpage").append("<a class='num' onclick='chosenthis("+i+")' href='users/changePage?num="+i+"'>"+i+"</a>");
	}
}
}
