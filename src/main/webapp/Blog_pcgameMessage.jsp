...
<html class="x-admin-sm">
   ...
   function searchmessage(fid){//searchmessage start
   	...
   	$.ajax({//ajax begin
		type:'GET',
     	url:'${pageContext.request.contextPath}/BPCgameM/getAllToolMessage?number='+fid,
     	dataType:'json',
     	async: false,
     	success:function(rs){ 		
     		$.each(rs,function(index,item){
               	...
               	$('#tbmessage').append("<tr>"
                     ...
                     +"<td>"+item.kissid+"</td>"
                     +"<td class='td-manage'>"    
                     +"<a title='编辑'  onclick=\"xadmin.open('编辑','Blog_pcgamemessage-edit.jsp?nid="+item.btid+"',600,400)\" href='javascript:;'>"
   ...
   function getcount(){
	   ...
	   $.ajax({
			type:'GET',
	     	url:'${pageContext.request.contextPath}/BPCgameM/howmanymessages',
	     	dataType:'json',
	     	async: false,
	     	success:function(rs){ 
	     		usersnumber = rs;
   ...
   $.ajax({
	   	type:"GET",
	   	url:"${pageContext.request.contextPath}/BPCgameM/returnPage",
	   	datatype:"json",
	   	async: false,
	   	success:function(rs){
	   		if(rs==1){
	   			...
	   			if(parseInt(usersnumber)<2){
	   				...
	   				$("#countpage").html("<a class='prev'  id='prev' onclick='goprev()'  href='javascript:;'>&lt;&lt;</a>"
	   						+"<a class='num'  href='BPCgameM/changePage?num=1'>首页</a>"        
	   						+"<span class='current' id='currentthis'>"+rs+"</span>"
	                        +"<a class='num' onclick='chosenthis("+(parseInt(rs)+1)+")' href='BPCgameM/changePage?num="+(parseInt(rs)+1)+"'>"+(parseInt(rs)+1)+"</a>"
	                        +"<a class='num' onclick='chosenthis("+(parseInt(rs)+2)+")'  href='BPCgameM/changePage?num="+(parseInt(rs)+2)+"'>"+(parseInt(rs)+2)+"</a>"
	                        +"<a class='num'  href='BPCgameM/changePage?num="+usersnumber+"'>尾页</a>" 
	   			...
	   			$("#countpage").html("<a class='prev' id='prev' onclick='goprev()' href=''>&lt;&lt;</a>"
	   				    ...
	   				    +"<a class='num' onclick='chosenthis("+(parseInt(rs)-1)+")' href='BPCgameM/changePage?num="+(parseInt(rs)-1)+"'>"+(parseInt(rs)-1)+"</a>"
                        +"<span class='current' id='currentthis'>"+rs+"</span>"
                        +"<a class='num'  href='BPCgameM/changePage?num="+(parseInt(rs)+1)+"'>"+(parseInt(rs)+1)+"</a>" 
                        +"<a class='num'  href='BPCgameM/changePage?num="+usersnumber+"'>尾页</a>" 
 ...
 function gonext(){
	...
	$("#next").attr("href","BPCgameM/changePage?num="+currentthis);
	}	
 }
 ...
 //回退翻页
  function goprev(){
	...
	if(currentthis<=1){
	$("#prev").attr("href","BPCgameM/changePage?num="+currentthis);
 ...
 $("#searchbutton").click(function(){ 
	 ...
	 $.ajax({		 
		 type:"POST",
		 url:"${pageContext.request.contextPath}/BPCgameM/substringBy",
...
for(var i=1;i<=4;i++){
	if(obj==i){
		...
		$("#countpage").append("<a class='num' onclick='chosenthis("+i+")' href='BPCgameM/changePage?num="+i+"'>"+i+"</a>");
	}
}
}
